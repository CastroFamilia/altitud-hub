...
export async function POST(request) {
  try {
    ...
    const { email, full_name, role = 'agent', team_id, remax_agent_id, remax_agent_name, office = 'altitud', avatar_url, phone } = body;
    ...
    if (existing) {
      return Response.json({ error: 'Este email ya está registrado', profile: existing }, { status: 409 });
    }
    ...
    // 5. Send invitation email via Supabase Admin API
    ...
    return Response.json({
      ...
      message: inviteSent 
        ...
        : 'Perfil creado. Configure SUPABASE_SERVICE_ROLE_KEY para enviar invitaciones por email.',
    });
