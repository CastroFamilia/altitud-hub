...
/* ═══════════════════════════════════════
   TEAM LEADER PANEL — Mi Equipo
   ═══════════════════════════════════════ */

export default function EquipoPage() {
  const { profile, isTeamLeader, supabase } = useAuth();
  const { t } = useApp();
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!myTeam) {
        ...
        const mockTeam = { id: 'mock-team', name: 'Equipo Alpha (Vista Previa)' };
        setTeam(mockTeam);
        setMembers([
  ...
  return (
    <>
      <TopNav title="Mi Equipo" subtitle={team ? team.name : "Gestión de equipo"} />
