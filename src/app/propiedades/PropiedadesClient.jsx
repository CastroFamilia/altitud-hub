...
export default function PropiedadesClient({ initialProperties = [] }) {
  const { t, lang } = useApp();
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [initialProperties, activeTab, searchQuery]);

  // Stats
  const stats = useMemo(() => ({
    total: initialProperties.length,
    draft: initialProperties.filter(p => p.status === 'draft').length,
    pending: initialProperties.filter(p => p.status === 'pending_approval').length,
    needs_changes: initialProperties.filter(p => p.status === 'needs_changes').length,
    approved: initialProperties.filter(p => p.status === 'approved').length,
    published: initialProperties.filter(p => p.status === 'published').length,
    sold: initialProperties.filter(p => p.status === 'sold').length,
  }), [initialProperties]);
          ...
          <div className="glass-panel rounded-2xl p-4 mb-6">
            <div className="flex flex-col md:flex-row md:items-center gap-3">
              ...
              <div className="flex items-center gap-1 flex-wrap">
                {TABS.map(tab => {
                  const count = tab.key === 'all' ? initialProperties.length : initialProperties.filter(p => p.status === tab.key).length;
                  return (
                    <button
                      key={tab.key}
