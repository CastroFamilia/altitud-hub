...
export default function AgentDataImportTab({ profiles = [] }) {
  ...
  const handleImport = async () => {
    if (!selectedAgent || !csvData || csvData.length === 0) return;
    setImporting(true);
    setResult(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-black italic text-slate-900 dark:text-white">
            📥 {lang === 'en' ? 'Data Import' : 'Importar Datos'}
          </h2>
      ...
      {selectedAgent && importType && (
        <div className="glass-panel rounded-2xl p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            >
              📄 {lang === 'en' ? 'Download Template' : 'Descargar Plantilla'}
            </button>
          </div>
          ...
          {!csvData ? (
            ...
            >
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {lang === 'en' ? 'Click to upload CSV' : 'Haz clic para subir CSV'}
              </p>
              <p className="text-[9px] text-slate-400 mt-1">.csv, .txt</p>
            </button>
          ) : (
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full font-bold">
                    {csvData.length} {lang === 'en' ? 'rows' : 'filas'}
                  </span>
                </div>
                <button
              ...
              {csvData.length > 5 && (
                <p className="text-[9px] text-center text-slate-400 mt-1">
                  +{csvData.length - 5} {lang === 'en' ? 'more rows' : 'filas más'}
                </p>
              )}
          ...
          {result.success ? (
            <div className="text-center">
              ...
              <p className="text-lg font-black text-emerald-600 dark:text-emerald-400 mt-2">
                {lang === 'en' ? 'Import Successful!' : '¡Importación Exitosa!'}
              </p>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                {result.imported} {lang === 'en' ? 'records imported' : 'registros importados'}
                {result.skipped > 0 && ` · ${result.skipped} ${lang === 'en' ? 'skipped' : 'omitidos'}`}
              </p>
              {result.errors && result.errors.length > 0 && (
                <div className="mt-3 text-left">
                  <p className="text-[10px] font-bold text-red-500 mb-1">{result.errors.length} {lang === 'en' ? 'errors' : 'errores'}:</p>
      ...
      <div className="glass-panel rounded-2xl p-5">
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">
          {lang === 'en' ? 'Import History' : 'Historial de Importaciones'}
        </h3>
        {loadingHistory ? (
          ...
          <p className="text-sm text-slate-400 text-center py-4">
            {lang === 'en' ? 'No imports yet' : 'Sin importaciones aún'}
          </p>
        ) : (
