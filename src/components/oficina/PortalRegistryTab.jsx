      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black text-slate-900 dark:text-white">
            {t('auto_portal_registry')}
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">
            {activeCount} {t('auto_active_portals')} · {portals.length} {t('auto_total_1')}
          </p>
        </div>
        <button
        ...
        {showAddForm && (
          <div className="glass-panel rounded-2xl p-5 border-2 border-brand-200 dark:border-brand-800">
            <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-4">
              {editingPortal ? (t('auto_edit_portal')) : (t('auto_new_portal'))}
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">{t('auto_display_name')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">{t('auto_category')}</label>
                <select
                  value={form.category}
                ...
                >
                  ...
                  <option value="on_request">{t('auto_on_request')}</option>
                </select>
              </div>
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">{t('auto_office')}</label>
                <select
                  value={form.office_scope}
                  onChange={e => setForm(f => ({ ...f, office_scope: e.target.value }))}
                  className="w-full text-sm bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-dark-border rounded-lg px-3 py-2 text-slate-700 dark:text-white"
                >
                  <option value="all">{t('auto_all_offices')}</option>
                  <option value="altitud">{t('ofc_portal_altitud')}</option>
                  <option value="cero">{t('ofc_portal_cero')}</option>
                </select>
              </div>
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">{t('auto_display_order')}</label>
                <input
                  type="number"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 dark:bg-white/5 border-b border-slate-100 dark:border-dark-border">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t('auto_category')}</th>
                <th className="text-left px-4 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t('auto_office')}</th>
                <th className="text-center px-4 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t('auto_active')}</th>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          title={t('auto_delete_permanently')}
                        >
