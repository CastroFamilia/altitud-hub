        ...
        <nav className={`flex-1 mt-4 md:mt-6 px-3 space-y-1 overflow-y-auto`}>
          ...
          {isAdminMode ? (
            ...
            <>
              ...
              {/* ── GESTIÓN DE EQUIPO (Team Leader / Broker) ── */}
              {(isTeamLeader || isBroker) && (
                <>
                  <div className="mt-6 mb-2 px-3 text-xs font-semibold text-gray-500 dark:text-gray-500 uppercase tracking-wider">Gestión de Equipo</div>
                  
                  <Link href="/oficina/lider" onClick={() => setMobileOpen(false)} className="nav-item flex items-center px-3 py-2.5 rounded-2xl text-gray-600 dark:text-gray-400 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors group">
                    <div className="w-6 h-6 mr-3 rounded bg-indigo-500/10 flex items-center justify-center group-hover:bg-indigo-500/20 transition-colors">
                      <svg className="w-4 h-4 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                    </div>
                    <span className="nexus-header text-[11px] text-gray-800 dark:text-white leading-none">Mi Panel de Equipo</span>
                  </Link>
                </>
              )}
