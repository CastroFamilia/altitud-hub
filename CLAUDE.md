...
- **Deployment:** Self-hosted via **Coolify** (Docker-based). No more `npx vercel --prod`.
...
- **Deployment:** Self-hosted on Coolify (replacing Vercel). Hub lives at `hub.remax-altitud.cr`.
- **Database:** Migrating from Supabase to self-hosted PostgreSQL. Existing `@supabase/*` imports are legacy and are being replaced by direct PostgreSQL clients/ORMs via the DAL.
...
- **Drive-Based Photos**: Property images are managed via Google Drive folders. The photographer uploads to Drive; the Hub syncs and distributes URLs to RECONNECT and portals. Photographer workflow tracking is natively embedded into the listing pipeline.
