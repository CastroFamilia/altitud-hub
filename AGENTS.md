...
- **Drive-Based Photos**: Property images are managed via Google Drive folders. The folder can be created automatically or manually associated at the **Pre-Listing** or **ACM (CMA)** phase (`acm_reports` table) as the **single base folder** to prevent duplicates, linked in the UI with an explainability modal. If an agent is working off-site without internet/signal, they can link an existing folder manually inside the wizards or directly from the dashboard lists (using the chain-link icon). The system extracts the folder ID via regex and bypasses automatic creation. The photographer uploads to Drive; the Hub syncs and distributes URLs to RECONNECT and portals. Photographer scheduling, WhatsApp coordination templates, and timeline tracking (`photos_requested_at`) are natively embedded into the pipeline.
